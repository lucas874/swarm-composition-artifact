...
def main():
    ...
    parser.add_argument('-s', '--subsizes', type=str, help='Folder containing subscription size results')
    parser.add_argument('-t', '--time', type=str, help='Folder containing execution time results')
    args = parser.parse_args()

    if not args.subsizes or not args.time:
        parser.print_help()
        sys.exit(1)

    result_directory =  f"results"
    execution_time_csv_filename = "general_pattern_microseconds_performance_results_edges.csv"
    cbor_to_csv(args.time, result_directory, execution_time_csv_filename)

    sub_size_csv_filename = "subscription_sizes_efrac.csv"
    subscription_results(args.subsizes, result_directory, sub_size_csv_filename)
